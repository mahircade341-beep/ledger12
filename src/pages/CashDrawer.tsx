  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="stat-v2"><span className="stat-label-v2">Drawdowns</span><span className="stat-value-v2 text-cyan-400">KES {totals.drawdown.toLocaleString()}</span></div>
        <div className="stat-v2"><span className="stat-label-v2">Restocks</span><span className="stat-value-v2 text-amber-400">KES {totals.restock.toLocaleString()}</span></div>
        <div className="stat-v2"><span className="stat-label-v2">Expenses</span><span className="stat-value-v2 text-red-400">KES {totals.expense.toLocaleString()}</span></div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="card-v2">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between p-2.5 bg-[var(--bg-surface2)] rounded-xl"><span className="text-[var(--text-secondary)] flex items-center gap-1.5"><svg className="w-3.5 h-3.5 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> Drawdowns</span><span className="text-cyan-400 font-semibold">KES {totals.drawdown.toLocaleString()}</span></div>
              <div className="flex justify-between p-2.5 bg-[var(--bg-surface2)] rounded-xl"><span className="text-[var(--text-secondary)] flex items-center gap-1.5"><svg className="w-3.5 h-3.5 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m8.25 3v6.75m0 0l-3-3m3 3l3-3M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg> Restocks</span><span className="text-amber-400 font-semibold">KES {totals.restock.toLocaleString()}</span></div>
              <div className="flex justify-between p-2.5 bg-[var(--bg-surface2)] rounded-xl"><span className="text-[var(--text-secondary)] flex items-center gap-1.5"><svg className="w-3.5 h-3.5 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg> Expenses</span><span className="text-red-400 font-semibold">KES {totals.expense.toLocaleString()}</span></div>
