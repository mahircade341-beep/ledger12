...
export default function Insights() {
  ...
  const exportCSV = useCallback(() => {
    ...
    const header = 'Date,Time,Items,Payment Method,Discount,Total\n';
    const rows = filteredTransactions.map((t: any) => `${new Date(t._creationTime).toLocaleDateString()},${new Date(t._creationTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })},${t.items.length},${t.paymentMethod},${t.discount},${t.total}`).join('\n');
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div><h1 className="page-title">Insights</h1><p className="page-subtitle">Sales analytics & anti-theft auditing</p></div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="stat-card"><span className="stat-label">Gross Sales</span><span className="stat-value text-emerald-400">KES {grossSales.toLocaleString()}</span><span className="text-xs text-[var(--text-muted)]">{numTransactions} transactions</span></div>
        ...
        <div className="stat-card"><span className="stat-label">Discounts Given</span><span className="stat-value text-amber-400">KES {totalDiscounts.toLocaleString()}</span></div>
        <div className="stat-card"><span className="stat-label">Cash Out (Payouts)</span><span className="stat-value text-red-400">KES {totalPayoutsAmt.toLocaleString()}</span></div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-[var(--text-muted)] border-b border-slate-200/60 dark:border-slate-800/60">
              <th className="text-left py-2 pr-2">Date</th><th className="text-right py-2 px-2">Items</th><th className="text-right py-2 px-2">Payment</th><th className="text-right py-2 px-2">Discount</th><th className="text-right py-2 pl-2">Total</th>
            </tr></thead>
            <tbody>
              ...
              {filteredTransactions.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-[var(--text-muted)] text-sm">No transactions in this period</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
